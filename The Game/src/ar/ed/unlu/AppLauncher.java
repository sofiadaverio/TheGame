...
public class AppLauncher {
    ...
    private static void iniciarCliente(String ipCliente, int puertoCliente, String ipServidor, int puertoServidor, String nombre) {
        try { // 1. Este es el try principal del método
            ...
            try { // 2. Este es el try para la validación de nombre
                // Primero intentamos la conexión lógica con el servidor
                controlador.agregarVista(nombre, vista);

                // SI LLEGÓ ACÁ es porque no hubo excepción, entonces iniciamos la vista
                vista.iniciar();

            } catch (Exception e) {
                // Si falla (por nombre duplicado u otro error de red),
                // mostramos el error y NO abrimos la vista principal.
                JOptionPane.showMessageDialog(null,
        ...
        } catch (Exception e) { // 3. Cierre del try principal (punto 1)
            e.printStackTrace();
